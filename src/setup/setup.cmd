...
:: Build
msbuild -Restore MetadataTask\MetadataTask.csproj -p:Configuration=%_C% -p:Platform=x64 -tl -nologo -m -warnaserror -bl:%_L%\setup_task.binlog || exit /b

msbuild -Restore setup.sln -p:Configuration=%_C% -p:Platform=x64 -tl -nologo -m -warnaserror -bl:%_L%\setup_build.binlog || exit /b
