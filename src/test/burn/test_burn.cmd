...
msbuild -Restore -p:Configuration=%_C% -p:Platform=x64 -tl -nologo -m -warnaserror -bl:%_L%\test_burn_build.binlog || exit /b
msbuild -Restore TestData\TestData.proj -p:Configuration=%_C% -p:Platform=x64 -tl -nologo -m -warnaserror -bl:%_L%\test_burn_data_build.binlog || exit /b

"%_B%\net462\win-x64\testexe.exe" /dm "%_B%\net6.0-windows\testhost.exe"
